...
/**
  ...
  */
class CassandraUserMessageRepository(server: ServerDesc, username: String, contactPoints: Set[InetAddress], keyspace: String = "ircsy",
  ...
  private def toPrivateMessage(row: Row): PrivateMessage = {
    ...
    val chat = new PrivateChat(row.getString("mainParticipant"), row.getString("secondParticipant"))
    ...
    new PrivateMessage(new ServerDesc(host, port), date, chat, author, message)
  }
  ...
  override def getPrivateMessages(chat: PrivateChat, from: LocalDateTime, to: LocalDateTime): util.List[PrivateMessage] = {
    val statement = lastUserMessageStatement.bind(chat.getMainParticipantName, chat.getSecondParticipantName,
  ...
  override def getLastPrivateMessages(chat: PrivateChat, count: Int): util.List[PrivateMessage] = {
    val statement = lastUserMessageStatement.bind(chat.getMainParticipantName, chat.getSecondParticipantName, count)
    executeMessageSelectStatement(statement)
  }
  ...
  private def executeMessageSelectStatement(statement: BoundStatement): util.List[PrivateMessage]  = {
    session.execute(statement).all()
      .parallelStream()
      .map(toJavaFunction(toPrivateMessage))
...
object CassandraUserMessageRepository {
  ...
  class Factory(contactPoints: Set[InetAddress]) extends UserMessageRepositoryFactory {
    override def newRepository(server: ServerDesc, username: String): UserMessageRepository =
      new CassandraUserMessageRepository(server, username, contactPoints)
  }
